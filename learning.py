...
def get_f_scores(threshold):
    ...
    for x in range(len(move_set1)):
        if  any(move_set1[x] in sublist for sublist in possibility) :
            ...
            if f <= threshold:
                if list_to_string(cube) not in before:
                    pre_open.append([move_set1[x],f,heuristic(cube,goal),list_to_string(cube),g])
            if f > threshold:
                pruned.append([move_set1[x],f,heuristic(cube,goal),list_to_string(cube),g])
            before.add(list_to_string(cube))
    ...
    for x in range(len(move_set2)):
        if  any(move_set2[x] in sublist for sublist in possibility) :
            ...
            if f <= threshold:
                if list_to_string(cube) not in before:
                    pre_open.append([move_set2[x], f, heuristic(cube, goal), list_to_string(cube),g])
            if f > threshold:
...
while continue_algo:
    ...
    if iteration !=1:
        cube=list_to_string(start)
        cube= string_to_2d_list(cube)
        open_state.clear()
